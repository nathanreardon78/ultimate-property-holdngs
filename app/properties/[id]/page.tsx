...
export default function PropertyDetail({ params }: { params: ParamsPromise }) {
  const { id } = use(params);
  const property = useMemo(() => properties.find((x) => x.id === id), [id]);
...
function PropertyDetailContent({ property }: { property: Property }) {
  ...
  const [unitGallery, setUnitGallery] = useState<{ unit: Unit; index: number } | null>(null);

  const heroImages = property.gallery.length
    ? [property.heroImg, ...property.gallery]
    : [property.heroImg];
  ...
  const stepUnitImage = (delta: number) => {
    setUnitGallery((current) => {
      if (!current || !current.unit.images?.length) return current;
      const total = current.unit.images.length;
  ...
  return (
    <div className={`${styles.container} space-y-6 pb-12`}>
      ...
      <div className="relative mt-4 overflow-hidden rounded-2xl border" style={{ height: '60vh' }}>
        <Image src={heroImages[heroIdx]} alt={property.name} fill className="object-cover" />
        {heroImages.length > 1 && (
          <>
            <button
      ...
      {heroImages.length > 1 && (
        <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-6">
          {heroImages.map((src, idx) => (
            ...
            >
              <Image src={src} alt={`${property.name} ${idx + 1}`} fill className="object-cover" />
            </button>
          ))}
        </div>
      )}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <div>
            ...
            <div className="mt-4 text-sm text-gray-700">
              Approx. {property.sqftApprox} • {property.bathsSummary}
            </div>
          </div>
          ...
          <section>
            ...
            <div className="mt-4 grid gap-3">
              {property.units.map((unit) => {
                const cover = unit.images?.[0] || unit.image || property.heroImg;
                return (
                  <div
                    key={unit.id}
                    role="button"
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                      <div className="flex flex-1 items-center gap-3">
                        <div className="relative h-16 w-24 overflow-hidden rounded-lg border">
                          <Image src={cover} alt={unit.label} fill className="object-cover" />
                        </div>
                        <div>
                          ...
                          <div className="mt-1 text-xs uppercase tracking-wide text-gray-500">
                            Click to view {unit.images?.length ?? 0} photos
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-start gap-3 sm:items-end sm:text-right">
                        ...
                        <a
                          ...
                          href={`/contact?property=${encodeURIComponent(property.id)}&unit=${encodeURIComponent(unit.label)}`}
        ...
        <aside className={`${styles.card} ${styles.cardPad}`}>
          ...
          <a
            ...
            href={`/contact?property=${encodeURIComponent(property.id)}`}
          >
            Send a Message
          </a>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-10 backdrop-blur-sm">
          <div className="relative w-full max-w-4xl rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="mt-2 text-sm text-gray-600">
              {unitGallery.unit.bedrooms} BR • {unitGallery.unit.bathrooms} BA •{' '}
              {unitGallery.unit.sqft} sqft
            </div>

            <div className="relative mt-6 h-[45vh] rounded-xl bg-black sm:h-[55vh]">
              {unitGallery.unit.images && unitGallery.unit.images.length > 0 ? (
                <>
                  <Image
                    src={unitGallery.unit.images[unitGallery.index]}
                    alt={`${unitGallery.unit.label} photo ${unitGallery.index + 1}`}
                    fill
                    className="object-contain"
                  />
                  {unitGallery.unit.images.length > 1 && (
                    <>
                      <button
            ...
            {unitGallery.unit.images && unitGallery.unit.images.length > 1 && (
              <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
                {unitGallery.unit.images.map((thumb, idx) => (
                  <button
                    key={thumb}
                  ...
                  >
                    <Image src={thumb} alt={`${unitGallery.unit.label} thumbnail ${idx + 1}`} fill sizes='(min-width: 640px) 16rem, 12rem' className="object-cover" />
                  </button>
                ))}
              </div>
            )}
